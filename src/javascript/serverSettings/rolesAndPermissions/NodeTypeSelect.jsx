 ...
 *
 * The list is flat on purpose. Core matches j:nodeTypes with isNodeType, which follows no single
 * hierarchy, so grouping the entries would state a structure the match does not have. The search is
...
const NodeTypeSelect = ({values, language, onChange}) => {
    ...
    const options = useMemo(() => {
        ...
        // A type the role already carries but the list does not offer, because its module was
        // uninstalled or because it was written by another tool. Dropping it from the options would
        // drop it from the value on the next save, so the role would silently widen.
        const names = new Set(nodes.map(nodeType => nodeType.name));
        const orphans = values.filter(value => !names.has(value)).map(value => ({
            label: t('rolesAndPermissions.detail.nodeTypeOutsideList', {name: value}),
            value,
