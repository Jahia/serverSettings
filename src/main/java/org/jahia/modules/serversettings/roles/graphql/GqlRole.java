...
public class GqlRole {
    ...
    @GraphQLField
    @GraphQLDescription("One target of this role by identity, or null when neither this role nor an "
            + "ancestor role declares it")
    public GqlRoleGrant getGrant(
            @GraphQLName("id") @GraphQLNonNull @GraphQLDescription("The target identity, empty for the "
                    + "node the role is granted on") String id) {
        return model.getGrantIds(role.getName()).contains(id) ?
                new GqlRoleGrant(model, role.getName(), id) :
                null;
    }
